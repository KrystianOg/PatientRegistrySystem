...
class TestAuthentication(TestsSetup):
    def test_can_patient_login(self):
        response = self.client.post(reverse("token_obtain_pair"), self.patient_data, format="json")
    ...
    def test_can_doctor_login(self):
        response = self.client.post(reverse("token_obtain_pair"), self.doctor_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue("access" in response.data)
        self.assertTrue("refresh" in response.data)
